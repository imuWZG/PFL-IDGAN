...
def PFLDGAN_train(train_dataloaders,testlables,test_dataloaders,clients_models,local_generators,local_discriminators,autoencoder,args,e):
    for xx in range(args.local_epochs):
        ...
        #本地生成器迭代
        for pid in range(args.clients_num):
            g,d=utils.generator_local_train2(local_generators[pid],local_discriminators[pid],train_dataloaders[pid],pid,args,'PFLDGAN')
            local_generators[pid].load_state_dict(torch.load(g))
            local_discriminators[pid].load_state_dict(torch.load(d))
...
def main():
    ...
    parser.add_argument("--gen_pro_epochs", type=int, default=15, help="Number of generator process epochs")
    ...
    parser.add_argument("--z_dim", type=int, default=250, help="dimensionality of the latent space")
    parser.add_argument("--clients_num", type=int, default=5, help="number of clients")
    parser.add_argument("--img_save_path", type=str, default='/workspace/algorithm/code/gan_images/CIFAR10/', help="image save path")
    parser.add_argument("--model_save_path", type=str, default='/workspace/algorithm/code/models/CIFAR10/', help="Generator model save path")
    parser.add_argument("--testset_sample_num", type=int, default=1000, help="Number of test sets")
    parser.add_argument("--encoder_path", type=str, default="/workspace/algorithm/code/models/CIFAR10-Encoder.pth", help="Encoder path")
    ...
    parser.add_argument("--PFLDGAN_generate_samples_num", type=int, default=100, help="")#300

    parser.add_argument("--dataset_name", type=str, default='CIFAR10', help="Path to dataset")
    ...
    parser.add_argument("--e", type=int, default=6.889, help="")
    parser.add_argument("--p_num", type=int, default=50, help="")
    args = parser.parse_args()

    #分割数据集，生成每个客户端的本地数据集
    subsets,testlables,Public_Set=utils.load_and_process_dataset2(args.dataset_name, args.dataset_root+args.dataset_name, args.clients_num,3000,args)
    ...
    for i in range(args.clients_num):
        ...
        utils.model_loading(PerFEDGAN_local_generator,
                            '/workspace/algorithm/code/models/CIFAR10/loading_models/participant'+str(i)+'Generator200.pth',
                            '/workspace/algorithm/code/models/CIFAR10/loading_models/participant'+str(i)+'Generator200.pth')
        utils.model_loading(PerFEDGAN_local_discriminator,
                            '/workspace/algorithm/code/models/CIFAR10/loading_models/participant'+str(i)+'Discriminator200.pth',
                            '/workspace/algorithm/code/models/CIFAR10/loading_models/participant'+str(i)+'Discriminator200.pth')
        '''
