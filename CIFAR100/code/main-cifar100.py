...
import utils
import argparse
from models import Generator,Discriminator,FeatureExtractor,Autoencoder2
...
def Local_train(train_dataloaders,test_dataloaders,clients_models,args):
    ...
    for index,clients_model in enumerate(clients_models):
        for i in range(args.local_epochs):
            clients_models[index].load_state_dict(utils.train(clients_model,args.device,train_dataloaders[index],args))
        accuracy = utils.test(clients_models[index],test_dataloaders[index],args)
        #print('客户端%d 的精度:%f%%' % (index,accuracy))
        all_clients_accuracys.append(accuracy)
    avg_acc=utils.calculate_average(all_clients_accuracys)
...
def FedAvg_train(train_dataloaders,test_dataloaders,global_model,clients_models,args):
    ...
    global_model=utils.average_model_weights(global_model.to(args.device),clients_models)
...
def FedProx_train(train_dataloaders,test_dataloaders,global_model,clients_models,args):
    ...
    new_global_state_dict=utils.fed_prox(clients_models, global_model.to(args.device).state_dict())
...
def FedMD_train(train_dataloaders,test_dataloaders,Public_Set,clients_models,args):
    ...
    clients_models=utils.fedmd_step(clients_models,Public_Set,args)
...
def PerFEDGAN_train(train_dataloaders,test_labels,test_dataloaders,clients_models,local_generators,local_discriminators,args,e):
    for xx in range(args.local_epochs):
        ...
        for pid in range(args.clients_num):
            g,d=utils.generator_local_train2(local_generators[pid],local_discriminators[pid],train_dataloaders[pid],pid,args,'PerFEDGAN')
    ...
    for i in range(args.clients_num):
        lables,samples = utils.generate_samples2(local_generators[i], args.PerFEDGAN_generate_samples_num, testlables[i], args)
    ...
    #混合样本和本地数据，训练本地任务模型
    gan_data_loaders=utils.distribute_samples_equally(all_lables,all_samples, args.clients_num,args)
    for index,clients_model in enumerate(clients_models):
        clients_models[index].load_state_dict(utils.train(clients_model,args.device,gan_data_loaders[index],args))
    ''' 
    #生成样本
    all_samples=[]
    all_labels=[]
    for i in range(args.clients_num):
        lables,samples = utils.generate_samples2(local_generators[i], args.PerFEDGAN_generate_samples_num, test_labels[i], args)
        for i, tensor in enumerate(samples):
            all_samples.append(tensor)
    ...
    all_clients_accuracys=[]
    for index, clients_model in enumerate(clients_models):
        clients_models[index].load_state_dict(utils.train(clients_model, args.device, mixed_dataloaders[index], args))
        accuracy = utils.test(clients_models[index],test_dataloaders[index],args)
        all_clients_accuracys.append(accuracy)
    avg_acc=utils.calculate_average(all_clients_accuracys)
    print('PerFEDGAN客户端平均精度:%f%%' % (avg_acc))
    ...
    for xx in range(args.local_epochs):
        ...
        for xxx in range(f_e):
            ...
            for pid in range(args.clients_num):
                ...
                local_discriminators[pid].load_state_dict(torch.load(d))
        '''         
            for w in range(args.clients_num):
                utils.save_model_checkpoint(local_generators[w],
                                            args.model_save_path+'local_generators'+str(w)+'epoch'+str(xxx)+'.pth',
                                            args.model_save_path+'local_generators'+str(w)+'epoch'+str(xxx)+'.pth')
            #utils.save_model_checkpoint(local_discriminators[w],args.model_save_path+'local_discriminators'+str(w)+'epoch'+str(xxx)+'.pth',args.model_save_path+'local_discriminators_b'+str(w)+'epoch'+str(xxx)+'.pth')
        '''
        ...
        all_clients_accuracys=[]
        for index,clients_model in enumerate(clients_models):
            new_dataloader=utils.data_generator(local_generators[index],train_dataloaders[index],args)
            clients_models[index].load_state_dict(utils.train(clients_model,args.device,new_dataloader,args))
            accuracy = utils.test(clients_models[index],test_dataloaders[index],args)
            all_clients_accuracys.append(accuracy)
        avg_acc=utils.calculate_average(all_clients_accuracys)
        ...
        for i in range(len(local_generators)):#相互学习
            ...
            while j<=len(local_generators)-1:
                local_generators[i],local_generators[j]=utils.adaptive_learning(local_generators[i],
...
def main():
    ...
    parser.add_argument("--gen_pro_epochs", type=int, default=25, help="Number of generator process epochs")
    parser.add_argument("--device", type=str, default='cuda', help=' ')
    parser.add_argument("--batch_size", type=int, default=32, help="size of the batches")
    parser.add_argument("--g_lr", type=float, default=0.00005, help="adam:Generator learning rate")
    parser.add_argument("--d_lr", type=float, default=0.00005, help="adam:Discriminator learning rate")
    parser.add_argument("--c_lr", type=float, default=0.000001, help="adam:Classifier learning rate")
    parser.add_argument("--z_dim", type=int, default=350, help="dimensionality of the latent space")
    parser.add_argument("--clients_num", type=int, default=5, help="number of clients")
    parser.add_argument("--img_save_path", type=str, default='/workspace/algorithm/code/gan_images/CIFAR100/', help="image save path")
    parser.add_argument("--model_save_path", type=str, default='/workspace/algorithm/code/models/CIFAR100/', help="Generator model save path")
    parser.add_argument("--testset_sample_num", type=int, default=600, help="Number of test sets")
    parser.add_argument("--encoder_path", type=str, default="/workspace/algorithm/code/models/CIFAR100-Encoder.pth", help="Encoder path")
    parser.add_argument("--adversarial_learning_epoch", type=int, default=100, help="The number of rounds in adversarial learning")#default=400
    parser.add_argument("--PerFEDGAN_generate_samples_num", type=int, default=100, help="")
    parser.add_argument("--PFLDGAN_generate_samples_num", type=int, default=100, help="")#default=300

    parser.add_argument("--dataset_name", type=str, default='CIFAR100', help="Path to dataset")
    ...
    parser.add_argument("--e", type=int, default=7.954, help="")
    parser.add_argument("--p_num", type=int, default=25, help="")
    ...
    #Public_Set=utils.generate_testset2(testlables, args.dataset_name, args.dataset_root+args.dataset_name, args.batch_size, 50)
    #根据客户端标签，生成每个客户端的测试集
    test_sets=[]
    ...
    #生成所有算法的客户端模型，依赖全局模型的生成全局模型
    #FedAvg
    FedAvg_global_model = utils.create_custom_cnn_2(args.dataset_name,'FedAvg')
    ...
    #FedProx
    FedProx_global_model = utils.create_custom_cnn_2(args.dataset_name,'FedProx')
    ...
    for i in range(args.clients_num):
        FedMD_clients_model=utils.create_custom_cnn_2(args.dataset_name,'FedMD')
    ...
    for i in range(args.clients_num):
        ...
        '''
        utils.model_loading(PerFEDGAN_local_generator,
                            '/workspace/algorithm/code/models/CIFAR100/loading_model/participant'+str(i)+'Generator500.pth',
                            '/workspace/algorithm/code/models/CIFAR100/loading_model/participant'+str(i)+'Generator500.pth')
        utils.model_loading(PerFEDGAN_local_discriminator,
                            '/workspace/algorithm/code/models/CIFAR100/loading_model/participant'+str(i)+'Discriminator500.pth',
                            '/workspace/algorithm/code/models/CIFAR100/loading_model/participant'+str(i)+'Discriminator500.pth')
        '''
        ...
        PerFEDGAN_clients_model=utils.create_custom_cnn_2(args.dataset_name,'PerFEDGAN')
    ...
    PFLDGAN_autoencoder = Autoencoder2().to(args.device)
    PFLDGAN_autoencoder.load_state_dict(torch.load(args.encoder_path))
    PFLDGAN_autoencoder.eval()
    for i in range(args.clients_num):
        PFLDGAN_clients_model=utils.create_custom_cnn_2(args.dataset_name,'PFLDGAN')
        ...
        '''
        utils.model_loading(PFLDGAN_local_generator,
                            '/workspace/algorithm/code/models/CIFAR100/loading_model/participant'+str(i)+'Generator500.pth',
                            '/workspace/algorithm/code/models/CIFAR100/loading_model/participant'+str(i)+'Generator500.pth')
        utils.model_loading(PFLDGAN_local_discriminator,
                            '/workspace/algorithm/code/models/CIFAR100/loading_model/participant'+str(i)+'Discriminator500.pth',
                            '/workspace/algorithm/code/models/CIFAR100/loading_model/participant'+str(i)+'Discriminator500.pth')
        '''
    ...
    for e in range(args.FL_epochs):
        ...
        Local_train(local_trainloaders,test_sets,Local_clients_models,args)
        ...
        with open('/workspace/algorithm/code/'+args.dataset_name+'FedAvg_avg_acc.csv', mode='a', newline='') as file:
        ...
        with open('/workspace/algorithm/code/'+args.dataset_name+'FedProx_avg_acc.csv', mode='a', newline='') as file:
            writer = csv.writer(file)
        ...
        with open('/workspace/algorithm/code/'+args.dataset_name+'FedMD_avg_acc.csv', mode='a', newline='') as file:
            writer = csv.writer(file)
        ...
        with open('/workspace/algorithm/code/'+args.dataset_name+'PerFEDGAN_avg_acc.csv', mode='a', newline='') as file:
            writer = csv.writer(file)
        ...
        PFLDGAN_clients_models,PFLDGAN_local_generators,PFLDGAN_local_discriminators,PFLDGAN_avg_acc=PFLDGAN_train(local_trainloaders,
                                      ...
                                      args,
                                      e)
        with open('/workspace/algorithm/code/'+args.dataset_name+'PFLDGAN_avg_acc.csv', mode='a', newline='') as file:
            writer = csv.writer(file)
