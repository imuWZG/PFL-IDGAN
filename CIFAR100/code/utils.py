...
def generate_proportions3(num_clients, num_classes, num_common_classes):
    ...
    for _ in range(num_clients):
        ...
        # 确定标签样本为0的类别数量
        zero_labels_count = random.randint(int(num_classes * 0.8), int(num_classes * 0.82))
...
def generate_proportions4(num_clients, num_classes, num_common_classes):
    if num_common_classes > num_classes:
        raise ValueError("The number of common classes cannot be greater than the total number of classes.")
    ...
    for _ in range(num_clients):
        ...
        random.shuffle(selected_classes)
        for class_idx in selected_classes[:-1]:
            max_available = min(0.3, remaining - 0.01 * (len(selected_classes) - selected_classes.index(class_idx) - 1))
            if max_available > 0.01:
                proportion = random.uniform(0.01, max_available)
                client_proportions[class_idx] = proportion
                remaining -= proportion
        ...
        # 最后一个选中的类别分配剩余的比例
        if remaining > 0:
            client_proportions[selected_classes[-1]] = min(0.3, remaining)
...
def load_and_process_dataset2(dataset_name, dataset_root, num_clients, samples_per_client, args):
    ...
    # 生成自定义客户端的数据比例
    num_common_classes=math.ceil(num_classes*0.03)
    proportions,common_classes = generate_proportions4(num_clients, num_classes,num_common_classes)
