...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(1, 16, 3, stride=2, padding=1)  # 输入通道数为1，输出通道数为16
...
def main():
    ...
    parser.add_argument("--model_save_path", type=str, default='/workspace/algorithm/code/models/FashionMNISIT-Encoder', help="Generator model save path")
    parser.add_argument("--dataset_root", type=str, default='/workspace/dataset/private/wzg_dataset/FashionMNIST', help="Path to dataset")
    ...
    train_dataset = datasets.FashionMNIST(root=opt.dataset_root, train=True, download=True, transform=transform)
    train_loader = DataLoader(train_dataset, batch_size=opt.batch_size, shuffle=True)

    # 加载验证集
    val_dataset = datasets.FashionMNIST(root=opt.dataset_root, train=False, download=True, transform=transform)
